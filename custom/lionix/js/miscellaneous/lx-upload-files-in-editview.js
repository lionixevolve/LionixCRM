    ...
    if ($("#" + element.field_name).length == 0) {
        ...
        $("#" + element.field_name)
            ...
            .append(
                '<button id="show_' +
                    element.field_name +
                    '_loader" type="button" class="btn btn-primary btn-sm">+ ' +
        ...
        $("#" + element.field_name)
            ...
            .append(
                '<div id="' +
                    element.field_name +
                    '_loader_hook" style="position: relative;"/>'
            );
    ...
    $.ajax({
        ...
        beforeSend: function (jqXHR, settings) {
            console.warn(
                ...
                "opportunities",
                "lx-upload-files-in-detailview.js",
        ...
        success: function (data, status, jqXHR) {
            ...
            $("#" + element.field_name + "_loader_hook").html(
                '<div id="floating-div-for-upload-' +
                    element.field_name +
            ...
            $("#floating-div-for-upload-" + element.field_name + "-file").show(
                "slow"
            );
            ...
            $("#exit-" + element.field_name + "-btn").click(function () {
                ...
                $("#show_" + element.field_name + "_loader").prop(
                    "disabled",
                    false
                );
                ...
                $("#" + element.field_name + "-form")
                    .ajaxForm({
                        ...
                        success: function (data, status, jqXHR) {
                            ...
                            console.warn("data:", data);
                            data = JSON.parse(data);
                            $("#preview_" + element.field_name).text(
                                data.message
                            );
                            $(
                                "#floating-div-for-upload-" +
                            ...
                            if (data.ok) {
                                ...
                                $(".lx-percent").html(percentVal);
                                $(
                                    "#exit-" + element.field_name + "-btn"
                                ).show();
                            } else {
                                $(".lx-progress").hide();
                                $("#" + element.field_name + "-file").show();
                                $(
                                    "#upload-" + element.field_name + "-btn"
                                ).show();
                                $(
                                    "#exit-" + element.field_name + "-btn"
                                ).show();
    ...
    try {
        ...
        if (lx.lionixCRM.config.allow_upload_files_fields) {
            ...
            if (crmEditView && !!crmEditView.module) {
                    ...
                    case "opportunities":
                        ...
                        if (text_fields_to_upload_fields_list.length) {
                            ...
                            if (lx.lionixCRM.config.debuglx) {
                                console.warn(
                                    "%s module have not upload fields configured.",
                                    module_name
                                );
                            }
                        }
                ...
                if (execute) {
                    console.warn(
                        "Rendering upload files fields for " +
                            crmEditView.module.value +
                            " module..."
                    );
    ...
    var observer = new MutationObserver(function (mutations) {
        if (mutations) {
            ...
            if (crmEditView && !!crmEditView.module) {
                if (lx.lionixCRM.config.debuglx) {
                    console.warn(
                        "Upload files in EditView observer for '%s' current module '%s' '%s' '%s'",
                        "all modules",
                        crmEditView.module.value,
                        "lx-upload-files-in-editview.js",
                        "!function()"
                    );
                }
                lx.upload.getFileFields(false);
